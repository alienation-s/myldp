...
for epsilon_w_D in epsilonDList:
    ...
    print(len(dataT_T))
    print(dataT_T[T_now-1:T_now+window-1])
    print(len(dataT_T[T_now-1:T_now+window-1]))
    print(dataResult)
    # 当前时刻DDHP均方误差MSE值
    # n_age原始数据频数,n_age_Noise加噪数据频数
    n_age, bins, patches = plt.hist(dataT_T[T_now-1:T_now+window-1], bins=list(range(0, 100, 10)),color='w')
    n_age_Noise, bins, patches = plt.hist(dataResult[T_now-1:T_now+window-1], bins=list(range(0, 100, 10)),color='w')
    print(n_age)
    print(n_age_Noise)
