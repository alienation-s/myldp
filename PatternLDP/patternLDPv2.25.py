...
# 总接口模块 process
def process(file_path, output_dir, w=160, total_budget=1.0, sample_fraction=1.0, delta=0.5, DTW_MRE=True):
...
def compare_experiments(file_path, output_dir, target):
    if target == "e":
        ...
        results = []
        for e in es:
            result_budget = process(file_path, output_dir, sample_fraction=1.0, total_budget=e, w=160, delta=0.5, DTW_MRE=True)
        ...
        results = []
        for w in ws:
            result_window = process(file_path, output_dir, sample_fraction=1.0, total_budget=1.0, w=w, delta=0.5, DTW_MRE=True)
