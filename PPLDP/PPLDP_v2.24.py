...
# Step 0: 统一数据处理接口
# =========================================================================
def process(file_path, output_dir, sample_fraction=1.0, w=160, total_budget=1.0):
    """
    ...
    # Step 1: 数据预处理
    data, _, = data_utils.preprocess_HKHS_data(file_path, sample_fraction)
    # data, _, = utils.preprocess_heartrate_data(file_path, sample_fraction)
    ...
    # Step 6: 显著点拟合曲线生成
    piecewise_fitted_values = generate_piecewise_linear_curve(
        data['date'], significant_indices, smoothed_values, data['date']
    )

    return normalized_data, smoothed_values, significant_indices, perturbed_values, piecewise_fitted_values
# =========================================================================
# Step 2: 显著点采样
# =========================================================================
...
def adaptive_w_event_budget_allocation(slopes, fluctuation_rates, total_budget, w, min_budget=1e-5):
    ...
    # 平滑 slopes 和 fluctuation_rates，避免异常波动影响
    slopes = np.clip(slopes, -3, 3)  # 使用较为合理的范围限制极端波动
    fluctuation_rates = np.clip(fluctuation_rates, 0, np.max(fluctuation_rates))  # 也限制波动率
...
def generate_piecewise_linear_curve(dates, significant_indices, values, original_dates):
    # 将日期转换为时间戳
    significant_dates = dates.iloc[significant_indices].astype('int64') / 1e9  # 转换为秒
...
def calculate_dtw_numpy(original_series, fitted_series):
    """
    使用 NumPy 计算原始时间序列与拟合时间序列的 DTW 距离
    参数:
        original_series (np.ndarray): 原始时间序列
        fitted_series (np.ndarray): 拟合后的时间序列
    返回:
        distance (float): DTW 距离
    """
    n, m = len(original_series), len(fitted_series)
    assert n == m, "Original and fitted series must have the same length for DTW calculation."
    ...
    for i in range(1, n + 1):
        for j in range(1, m + 1):
            cost = abs(original_series[i - 1] - fitted_series[j - 1])
...
if __name__ == "__main__":
    ...
    # 调用 process_hkhs_data 函数
    normalized_data_100, sample_100_smoothed_values, significant_indices_100, perturbed_values_100, piecewise_fitted_values_100 = process(file_path, output_dir, 1.0, 160, 1.0)
    ...
    # 计算拟合曲线与原始数据之间的 DTW 距离
    dtw_distance_100 = calculate_dtw_numpy(normalized_data_100, piecewise_fitted_values_100)
    dtw_distance_80 = calculate_dtw_numpy(normalized_data_80, piecewise_fitted_values_80)

    # 输出 DTW 距离
    print(f"DTW distance for sample_fraction=1.0: {dtw_distance_100:.4f}")
    print(f"DTW distance for sample_fraction=0.8: {dtw_distance_80:.4f}")


    # 输出显著点结果
    mre_100 = data_utils.calculate_mre(sample_100_smoothed_values, normalized_data_100[significant_indices_100])
    mre_80 = data_utils.calculate_mre(sample_80_smoothed_values, normalized_data_80[significant_indices_80])
