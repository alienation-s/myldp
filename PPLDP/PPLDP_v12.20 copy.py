...
def adaptive_significant_points(data, max_search=10, min_alpha=0.2, max_alpha=1.0, slope_diff_threshold=0.1, min_distance=5):
    ...
    while i < len(data) - 2:
        ...
        for j in range(len(diff)):
            if ((diff[j] >= 0 and (j == 0 or diff[j - 1] < 0)) or
                ...
                if candidate - significant_points[-1] >= min_distance:
                    ...
                    print(f"显著点采样 - 导数条件满足，选择点索引: {candidate}, 日期: {hkhs_data['date'].iloc[candidate]}")
                    break
        ...
        for j in range(i + 1, min(i + max_search, len(data))):
            ...
            # 计算下一点的斜率变化
            if j + 1 < len(data):
                next_slope = (data[j + 1] - data[j]) / 1  # 假设时间间隔为1
            ...
            print(f"正在评估点索引: {j}, slope: {slope:.4f}, next_slope: {next_slope:.4f}, tan_theta: {tan_theta:.4f}, threshold: {threshold:.4f}")
            ...
            if tan_theta > threshold:
                ...
                if candidate - significant_points[-1] >= min_distance:
                    ...
                    i = candidate  # 更新起始点
                    found_significant_point = True
                    print(f"显著点采样 - 斜率变化条件满足，选择点索引: {candidate}, 日期: {hkhs_data['date'].iloc[candidate]}")
                    break
        ...
        if not found_significant_point:
            ...
            print(f"显著点采样 - 强制选择点索引: {i}, 日期: {hkhs_data['date'].iloc[i]}")
            # 不更新 fluctuation_rate 以避免误判

        # 更新波动率
        if j + 1 < len(data):
            fluctuation_rate = abs(next_slope - slope)
            print(f"更新波动率: {fluctuation_rate:.4f}")

    significant_points.append(len(data) - 1)  # 添加终点
    print(f"最终显著点数量: {len(significant_points)}")
